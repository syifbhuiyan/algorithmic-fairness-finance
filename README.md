...
- **Bias Detected:**  
  The baseline model disproportionately penalized male applicants, flagging **90%** as *“high risk”* compared to **14%** of females.

- **Bias Mitigated:**  
  Using a `ThresholdOptimizer` with constraints for **Demographic Parity**, we successfully equalized the selection rates to approximately **41%** for both groups.
...
- **fairness_analysis.ipynb**  
  The complete Python code for the model and audit.

- **research_note.md**  
  A detailed academic note discussing the IS (Information Science) implications of the results.

---

## 🚀 How to Run

1. Open `fairness_analysis.ipynb` in **Google Colab** or **Jupyter Lab**.
2. Ensure you have a Kaggle API token (`kaggle.json`) if re-downloading the raw data.
